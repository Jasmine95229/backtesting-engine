...
Entry: Price breaks above the highest high (Long) or below the lowest low (Short)
       over a rolling lookback window, confirmed by a minimum ATR threshold to
       filter low-volatility false breakouts.
 
Exit: ATR-based initial stop loss, trailing stop with activation threshold,
      optional ATR take-profit, optional max holding bars.
 
Filters:
    - ATR confirmation: range of breakout bar must exceed min_atr_multiplier * ATR
    - Volume spike: optional filter requiring above-average volume on breakout bar
...
class ATRBreakout:
    ...
    def prepare_backtest_data(self):
        ...
        # --- Breakout signals ---
        # Use prior N bars (shift 1 to avoid look-ahead on current bar)
        rolling_high = mid_high.shift(1).rolling(window=self.breakout_window).max()
        rolling_low = mid_low.shift(1).rolling(window=self.breakout_window).min()

        # Breakout: close exceeds prior window high/low
        breakout_long = mid_close > rolling_high
        breakout_short = mid_close < rolling_low

        # ATR confirmation: current bar rnge must be meaningful
        bar_range = mid_high - mid_low
        atr_confirmed = bar_range > self.min_atr_multiplier * data['ATR']

        data['BreakoutLong'] = breakout_long & atr_confirmed
        data['BreakoutShort'] = breakout_short & atr_confirmed

        # Delay 1 bar - enter on next bar's open
        data['Long_entry_signal'] = data['BreakoutLong'].shift(1).fillna(False)
        data['Short_entry_signal'] = data['BreakoutShort'].shift(1).fillna(False)

        # Session filter
        if self.trade_start_hour is not None and self.trade_end_hour is not None:
            hour = data.index.hour
            is_session = (hour >= self.trade_start_hour) & (hour < self.trade_end_hour)
            data['Long_entry_signal'] = data['Long_entry_signal'] & in_session
            data['Shory_entry_signal'] = data['Short_entry_signal'] & in_session
        ...
        self.signal_data = self.backtest_data[[
            ...
            'Leverage', 'BreakoutLong', 'BreakoutShort', 'ATR', 'Long_BidHigh', 'Short_AskLow'
        ]]
    ...
    def update_signal(self, bar_idx, temp_, close_all=True):
        """Per-bar signal update: breakout entries with ATR trailing stops."""
        ...
        for d_num in temp_.direction_index.keys():
            ...
            if open_ids != 'failed':
                ...
                if d_num == 'Long':
                    temp_.highest_favorable_price[open_ids, d_n] = np.maximum(temp_.highest_favorable_price[open_ids, d_n], current_price)
                    ...
                    sl_hit = (temp_.sl_price[open_ids, d_n] > 0) & (temp_.sl_price[open_ids, d_n] >= sig[bar_idx, si[f'{d_num}_exit_price']])
                else:
                    temp_.highest_favorable_price[open_ids, d_n] = np.where(temp_.highest_favorable_price[open_ids, d_n] == 0, current_price,
                ...
                # Ratchet stop
                if d_num == 'Long':
                    temp_.sl_price[open_ids, d_n] = np.where(temp_.trail_active[open_ids, d_n],
                ...
                if self.max_holding_bars is not None:
                    ...
                    temp_.exit_type[open_ids, d_n] = np.where(held_too_long & (temp_.exit_type[open_ids, d_n] == 0),
                                                            -2,
                                                            temp_.exit_type[open_ids, d_n])

            # --- Signal-based exits ---
            if sig[bar_idx, si[f'{d_num}_exit_signal']]:
                exit_ids = (find_all_ if close_all else find_first_)(temp_.open_position[:, d_n], True)
                if exit_ids != 'failed':
                    temp_.exit_signal[exit_ids, d_n] = True
            else:
                exit_ids = (find_all_ if close_all else find_first_)(temp_.open_position[:, d_n], True)
                if exit_ids != 'failed':
                    temp_.exit_signal[exit_ids, d_n] = np.where(temp_.exit_type[exit_ids, d_n] != 0, True, False)

            # --- Set prices ---
            temp_.current_price[:, d_n] = sig[bar_idx, si[f'{d_num}_current_price']]
            temp_.entry_price[:, d_n] = sig[bar_idx, si[f'{d_num}_entry_price']]
            temp_.exit_price[:, d_n] = np.where(temp_.exit_type[:, d_n] == -1, temp_.sl_price[:, d_n],
                                                np.where(temp_.exit_type[:, d_n] == 1, temp_.tp_exit_price[:, d_n],
                                                np.where(temp_.exit_type[:, d_n] == -2, sig[bar_idx, si[f'{d_num}_current_price']],
